...
void printTable(struct sttable *tab, int n, FILE *fsymboltable)
{
    for (int i = 0; i < n; i++)
        fprintf(fsymboltable, "%d %s %s %s %d %d\n", tab[i].sno, tab[i].lexeme, tab[i].dtype, tab[i].type, tab[i].size, tab[i].args);
}
